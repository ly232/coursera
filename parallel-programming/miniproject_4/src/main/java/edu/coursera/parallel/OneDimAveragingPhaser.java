        ...
        for (int ii = 0; ii < tasks; ii++) {
            ...
            threads[ii] = new Thread(() -> {
                ...
                for (int iter = 0; iter < iterations; iter++) {
                    ...
                    /////
                    // Note: the point of using phaser to split arrive vs. awaitAdvance is so that the enclosed
                    // code block can execute IN PARALLEL with the awaiting operation. This implies the works in
                    // this section must be local to the thread task, which is why the boundary conditions are
                    // moved above the ph.arrive() call.
