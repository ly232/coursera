...
trait NodeScala {
  ...
  def start(relativePath: String)(handler: Request => Response): Subscription = {
    val httpListener = createListener(relativePath)
    val subscription = httpListener.start()
    Future.run(){
      token => Future{
        while (token.nonCancelled)
          httpListener.nextRequest onSuccess {
            case (request, exchange) => respond(exchange, token, handler(request))
          }
        // Caller canceled token. Now signal httpListener to cancel as well:
