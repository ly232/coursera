...
package object nodescala {
  ...
  implicit class FutureCompanionOps[T](val f: Future.type) extends AnyVal {
     ...
     */
    def always[T](value: T): Future[T] = Future(value)  // No {} necessary since value is already evaluated.
     ...
     */
    def all[T](fs: List[Future[T]]): Future[List[T]] = fs.foldRight(Future(Nil: List[T]))(
        (f, acc) => for {
          x <- f  // x is type T; f is type Future[T]
          xs <- acc  // xs is type List[T]; acc is type Future[List[T]]
        } yield x::xs)
     ...
     */
    def delay(t: Duration): Future[Unit] = blocking {
      val p = Promise[Unit]()
     ...
     *  This method creates a cancellation token source and runs
     *  a block of code f that takes a cancellation token.
     */
