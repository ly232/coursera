...
package object nodescala {
  ...
  implicit class FutureCompanionOps(val f: Future.type) extends AnyVal {
     ...
     *  Note: cannot just return Future(value) as the unit test shows
     *  it expectes always to block.
     */
    ...
    def never[T]: Future[T] = {
      val p = Promise[T]()
      p.future  // p.complete is never called, so p.future never finishes.
    }
    ...
    def all[T](fs: List[Future[T]]): Future[List[T]] =
      fs.foldRight(Future(List[T]()))((futureElem, futureList) => {
        for {
          elem <- futureElem
          list <- futureList
        } yield {
     ...
     */
    def delay(t: Duration): Future[Unit] = Future(blocking {
  ...
  implicit class FutureOps[T](val f: Future[T]) extends AnyVal {
    ...
    def continueWith[S](cont: Future[T] => S): Future[S] = {
      val p = Promise[S]()
      f onComplete { case _ => p complete Try(cont(f)) }
      p.future
    }
